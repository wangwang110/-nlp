...
def load_model():
    return cPickle.load(open("lm3.bin", 'r'))
...
def get_candidates(word):
    ...
    ## 词不在离线计算的文件中，重新计算加入
        ...
        for w in dist1:
            candidates[word].append((w,1,lcs.find_lcsubstr(w,word)/len(word),lcs.find_lcseque(w,word)/len(word)))
        for w in dist2:
            candidates[word].append((w,0.5,lcs.find_lcsubstr(w,word)/len(word),lcs.find_lcseque(w,word)/len(word)))
