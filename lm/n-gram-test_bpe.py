...
if __name__ == "__main__":
    ...
    def is_normal_text(self, text):
        # words = set(text.split(" "))
        # error_words = words - self.detect_dicts
        # if len(words) - len(error_words) >= len(error_words):
        #     return True
        # bpe_error_words = get_bpe_words(self.bpe, error_words)
        # for w in bpe_error_words:
        #     if len(w.split(" ")) >= 5:
        #         return False

        prob = self.judge_object.sentence_log_prob(list(text))
        if sum(prob)/len(text) <= -4.2:
            return False
        return True
